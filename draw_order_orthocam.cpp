...
{
  IrrlichtDevice *device = createDevice(video::EDT_OPENGL, dimension2d<u32>(SCREEN_WIDTH, SCREEN_HEIGHT), 32, false, false, false, NULL);
  ...
  if(device)
  {
    ...
    // Add a camera node
	irr::core::vector3df camPos((f32)screenSize.Width / 2, -((f32)(screenSize.Height / 2)), -0.1);
