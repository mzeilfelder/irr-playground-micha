...
//
// Trying to use a shader for the 2D materials.
// Irrlicht does not allow using shader-materials for 2D so far, so this is not yet working.
// See http://irrlicht.sourceforge.net/forum/viewtopic.php?f=4&t=52531 for discussion.
...
// fragment shader for UI
const char * Render2Dfsh = 
...
class Shader2DCallBack : public video::IShaderConstantSetCallBack
{
public:
	Shader2DCallBack () 
		: TextureUsageID(-1)
				...
				video::IMaterialRendererServices* services)
	{
		...
		services->setPixelShaderConstant(TextureUsageID, &textureUsage, 1);
	}
			...
			s32 userData)
	{
		...
		if (FirstUpdate)
		{
			...
			TextureUsageID = services->getPixelShaderConstantID("uTextureUsage");
			s32 TextureUnitID = services->getPixelShaderConstantID("uTextureUnit");

			s32 TextureUnit = 0;
			services->setPixelShaderConstant(TextureUnitID, &TextureUnit, 1);
...
int main()
{
	...
	// setup a 2d render shader
	video::IGPUProgrammingServices* gpu = driver->getGPUProgrammingServices();
	Shader2DCallBack* cbRender2D = new Shader2DCallBack();
	s32 material2D = gpu->addHighLevelShaderMaterial(Render2Dvsh, "main", EVST_VS_2_0,
		Render2Dfsh, "main", EPST_PS_2_0,
		cbRender2D, video::EMT_SOLID, 0);
	driver->getMaterial2D().MaterialType = (video::E_MATERIAL_TYPE)material2D;
