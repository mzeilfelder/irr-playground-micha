			...
			if ( event.GUIEvent.EventType == gui::EGET_BUTTON_CLICKED )
			{
				if ( event.GUIEvent.Caller->getID() == GUI_ID_BUTTON_SET_SPLINE_ANIMATOR )
				{
					if ( MeshSceneNode )
					{
						...
						bool loop=true;
						bool pingpong=true;
						scene::ISceneNodeAnimator * animator = Device->getSceneManager()->createFollowSplineAnimator(currentTime, points, speed, tightness, loop, pingpong);
