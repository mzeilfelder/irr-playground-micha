...
const float ROOM_RADIUS = 1200.f;	// quadratic space - outside nothing exists
...
scene::SMeshBuffer* CreateExtrudedPolyLineMeshBuffer(s32 numVertices_, const core::vector3df * vertices_, const core::vector3df &extrude_)
{
	...
	//    scene::IMeshManipulator * mm = sceneManager->getMeshManipulator();
...
struct Invaders
{
	Invaders() : mModel(NULL) {}
	scene::ISceneNode * mModel;
};

// What whoever controls the thing that moves wishes to do
struct Controller
{
	Controller()
	{
...
struct Player
{
	Player() : mLives(0), mScore(0), mModel(0)	{ }
...
struct Game
{
	...
	void CreateMeshes();
	void CreateModels(scene::ISceneManager* sceneManager);
	...
	s32 mPain;	// Player is in pain for a few ms after being hit
	s32 mNoStartTime;
...
void Game::CreateMeshes()
{
	...
	mMeshShip->addMeshBuffer(CreateShipMeshBuffer());

	mMeshAlien = new scene::SMesh();
	mMeshAlien->addMeshBuffer(CreateAlienMeshBuffer());

	mMeshShot = new scene::SMesh();
	mMeshShot->addMeshBuffer(CreateShotMeshBuffer());
}

void Game::CreateModels(scene::ISceneManager* sceneManager)
{
...
{
	// Player moving ship
	float movePlayer = (float)time_* 1.f;	// speed (or step this frame)
	if ( mController.mMoveRight )
	{
	...
	if ( mController.mShoot )
	{
		...
		for ( s32 i=0; i < MAX_SHOTS_PLAYER; ++i )	// find a free bullet (one which isn't flying yet)
		{
			if ( !mShotsPlayer[i].mModel->isVisible() )
			{
				// Start position for shot - a few pixels above ship
				mShotsPlayer[i].mModel->setPosition( mPlayer.mModel->getPosition() + core::vector3df(0, 10, 0) );
				mShotsPlayer[i].mModel->setVisible(true);
				break;
			}
		}
		mController.mShoot = false;
	}

	// Movement of invaders
	bool invadersHitFloor = false;
	float speedInvader = 0.1f + (float)(INVADER_ROWS*INVADER_LINES-mInvadersLiving)*0.005f + (float)mLevel*0.01f;
	float moveInvader = (float)time_* speedInvader;	// step they move this frame
	...
	for ( s32 i=0; i < INVADER_ROWS*INVADER_LINES; ++i )
	{
		if ( mInvaders[i].mModel->isVisible() )
		{
			...
			// Check if any invader reached the border
	...
	if ( flipDir )
	{
		// when an invader reached the border all start moving in other direction now
		mInvaderMoveDir *= -1;
		moveInvader *= -1;

		// They also all move one step closer to the bottom
		float moveDown = 50.f;
		for ( s32 i=0; i < INVADER_ROWS*INVADER_LINES; ++i )
		{
			if ( mInvaders[i].mModel->isVisible() )
	...
	// Invaders shoot once in a while (pretty regular - could also make it more random)
	...
	for ( s32 i=0; i < MAX_SHOTS_PLAYER; ++i )
	{
		if ( mShotsPlayer[i].mModel->isVisible() )
		{
			...
			{
				// Top reached - bullet is gone
				mShotsPlayer[i].mModel->setVisible(false);
			}
		}
	}

	// Move shots from invaders
	...
	for ( s32 i=0; i < MAX_SHOTS_INVADERS; ++i )
	{
		if ( mShotsInvaders[i].mModel->isVisible() )
		{
			...
			{
				// bottom reached - bullet is gone
				mShotsInvaders[i].mModel->setVisible(false);
			}
		}
	}

	// check collisions (player bullets)
	...
	// check collisions (invader bullets)
	for ( s32 i=0; i < MAX_SHOTS_INVADERS; ++i )
	{
		if ( mShotsInvaders[i].mModel->isVisible() )
		{
...
public:
	virtual bool OnEvent(const SEvent& event)
	{
		if ( event.EventType == irr::EET_KEY_INPUT_EVENT )
		{
			...
			{
				case KEY_ESCAPE: // allow leaving program with ESC
				...
				case KEY_SPACE:
					if ( event.KeyInput.PressedDown )
					{
						...
						{
							if ( GAME.mNoStartTime <= 0 )	// prevent accidental restart (shoot and restart use same keys)
							{
								GAME.StartGame();
							}
						}
						else
							GAME.mController.mShoot = true;
					}
				break;
				case  KEY_LEFT:
					GAME.mController.mMoveLeft = event.KeyInput.PressedDown;
				break;
				case KEY_RIGHT:
					GAME.mController.mMoveRight = event.KeyInput.PressedDown;
				break;
				default:
				break;
			}
...
int main()
{
	...
	GAME.CreateMeshes();
	GAME.CreateModels(sceneManager);
	...
	while(device->run() && driver && !GAME.mQuitGame)
	{
		if (device->isWindowActive())
		{
			...
			if ( GAME.mPain > 0 )
			{
				driver->beginScene(true, true, video::SColor(100,255,0,0));	// show that pain!
				GAME.mPain -= timeDiff;
			}
			else
			{
